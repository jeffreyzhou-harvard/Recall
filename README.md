...
- **Family search and Q&A:** ask questions in Connections, find shared stories, and get conversation ideas powered by Muse Spark. Answers link to attributed sources and their photo groups. Without a Muse key, source search remains available.
...
1. Click **Explore a sample family** on the landing page. Each click starts a fresh sample household with **Home Garden Morning** and **Quiet Library Rooms** only. The four person-focused moments, People groups, stories, and call history are absent. Connections already includes explicitly fictional family ties.
...
4. Those four groups also carry the fictional material in [fixtures/sample-family-demo.json](fixtures/sample-family-demo.json): six family-written stories on the Stories page, and 26 earlier calls behind **Recall sessions**. The per-topic record therefore shows a topic recalled unaided in 6 of 8 calls, one with too few calls to count, and a change line in each direction; **Print or save a PDF** renders it as a document for a doctor. The stories are each a named family member's own account and the history holds no transcript, contribution, or claim in the patient's name.
5. In this sample only, People shows at most **Grandmother, Mother, Daughter 1, and Daughter 2**, using authored crops from uploaded kit photos. These are demonstration fixtures, not inferred identities. Real households use opt-in face grouping.
6. Click **Try the patient call demo** to open the paired patient tab. It joins the same sample household through a separate signed session. Start the demo call, answer, and allow microphone access.
7. The sample call shows the event's photos with its opening question, displays live captions, and handles replies through the existing conversation gates. After audio playback, confirm storage and then sharing. Only share-confirmed words appear as a story on the associated moment and in the caregiver's Connections view.
...
Families can search and ask questions about their shared collection and accessible graph sources. Muse Spark supplies cited answers and ideas for conversations together; private, unshared patient accounts stay outside this search. Questions and generated answers are not saved as memories. Call records contain per-topic counts, dates, and fixed explanatory copy. The printed record states that it is **not a clinical assessment or diagnosis**.
