...
export type Principal =
  | { role: "operator"; member_id: null }
  | { role: "family" | "patient"; member_id: string };
...
export function credentials(): {
  ...
  try {
    const members: unknown = JSON.parse(
      process.env.RECALL_FAMILY_CREDENTIALS || "{}",
    );
    if (
      !members ||
      typeof members !== "object" ||
      Array.isArray(members) ||
      process.env.RECALL_FAMILY_SECRET
    )
      return null;
    const entries = Object.entries(members);
    if (
      entries.some(
        ([id, key]) => !id.trim() || typeof key !== "string" || !key.trim(),
      )
    )
      return null;
    ...
    if (
      ...
      (operator && keys.includes(operator))
    )
      return null;
...
export function sameOrigin(request: Request): boolean {
  ...
  // Next may normalize the URL host to localhost; the actual Host still identifies the browser origin.
  try {
    const supplied = new URL(origin);
    const host = request.headers.get("host");
    if (supplied.host === host && supplied.protocol === internal.protocol)
      return true;
    const publicUrl =
      process.env.RECALL_PUBLIC_URL || process.env.RELAY_PUBLIC_URL;
    return (
      !!publicUrl &&
      supplied.origin === new URL(publicUrl).origin &&
      supplied.host === host
    );
  } catch {
    return false;
...
export function principalForKey(key: string): Principal | null {
  ...
  for (const [id, secret] of Object.entries(c.members))
    if (equal(key, secret)) return { role: "family", member_id: id };
...
function signingKey(
  ...
  return principal.role === "operator"
    ...
    : principal.role === "family" &&
        Object.hasOwn(c.members, principal.member_id)
      ? c.members[principal.member_id]
      : (() => {
          const a = accountForMember(principal.member_id);
  ...
  try {
    ...
    if (
      (data.role !== "operator" &&
        data.role !== "family" &&
        data.role !== "patient") ||
      (data.role !== "operator" && typeof data.member_id !== "string") ||
      (data.role === "operator" && data.member_id !== null) ||
      !Number.isFinite(data.exp) ||
      data.exp <= Date.now() / 1000 ||
      data.exp > Date.now() / 1000 + lifetime
    )
      return null;
    const principal = {
      role: data.role,
