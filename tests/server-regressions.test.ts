...
describe("member-bound route access (§2 rules 5, 14, 15)", () => {
  ...
  it("serves the credential owner and enforces revocation on their next request", async () => {
    ...
    vi.mocked(getLiveRecall).mockResolvedValue({ ...rig, knowledge: new KnowledgeUpdater(rig.graph, () => rig.setup.current()), callMode: "none", media: null, currentCall: () => null, refreshSetup: async () => {}, tick: async () => null });
