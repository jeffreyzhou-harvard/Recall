...
import { familyShares, topicRecord } from "@/lib/recall-preview/model";
...
export default function FamilyPage() {
  ...
  const shares = familyShares(state, data.familyMember);
    ...
    <div className="recall-family-shell care-dashboard">
      ...
      <main className="care-main">
        <header className="care-page-heading">
          <div><h1>Susan, this week.</h1><p>A little context for your next conversation.</p></div>
        ...
        <section className="care-session-record" id="topic-record" aria-labelledby="record-title">
          ...
          {showRecord ? <SessionWaveform sessions={sessions}>
            <details className="session-week-note">
              <summary>This week’s note</summary>
              <p>{share?.topicId === "lincoln" ? copy["FAM-NOTE-02"] : copy["FAM-NOTE-01"]}</p>
              {share ? <><blockquote>“{share.text}”</blockquote><p className="care-caption">Susan chose to share with Maya · September 19</p><details className="care-source"><summary>Source & permission</summary><p>Scripted sample · {share.sourceLineId}. Both remember and share choices were confirmed in the preview. No audio was recorded.</p></details></> : <p className="care-caption">No personal words shared yet. Only words Susan chooses to share appear here.</p>}
            </details>
          </SessionWaveform> : <div className="care-overview"><p className="care-record-hidden">Session details are hidden. You can still suggest a conversation.</p><MemorySuggestionForm /></div>}
        ...
        <section className="care-sources" aria-labelledby="setup-summary-title">
          ...
          <p>{setup.agreed ? setup.photos.length + " photos · " + setup.contacts.length + " approved people · " + setup.events.length + " calendar events" : "Choose photos, approved people and occasions to talk about."}</p>
          {setup.agreed && <details><summary>Review selected sources</summary><ul>{setup.photos.map((photo, index) => <li key={photo.name + index}>Photo: {photo.name}</li>)}{setup.contacts.map((contact) => <li key={contact.id}>Person: {contact.name}{contact.relationship ? " · " + contact.relationship : ""}</li>)}{setup.events.map((event) => <li key={event.id}>Event: {event.title} · {event.date}</li>)}</ul><p>Calling window: {setup.days.join(", ")} · {setup.start}–{setup.end}</p><p>These selections remain local to the preview.</p></details>}</div>
