import { credentials } from "@/server/session";
...
async function change(request: Request, context: { params: Promise<{ id: string }> }, remove: boolean) {
  ...
  const { id } = await context.params, body = await bodyOf(request);
  ...
  return Response.json({ key, revoked: remove }, { headers: { "Cache-Control": "no-store" } });
}
