...
import { liveRecall, familyResponse, familyHousehold } from "../shared";
export const runtime = "nodejs";
export async function POST(request: Request) {
  const principal = browserPrincipal(request), household = familyHousehold(request);
  ...
  return familyResponse(async () => {
    ...
    const live = await liveRecall(request); live.currentCall()?.stop();
