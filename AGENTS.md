# AGENTS.md - Recall (HackMIT 2026)

**2026-09-20 base graph questions.** The user explicitly authorized answering family questions from the base graph even when no story exists. The query projection may include people, places, events and explicit connections from the shared collection, approved joint setup and the member's own confirmed graph contributions. Keep each source's attribution and relationship direction; a photo-group link is not evidence of attendance. Unshared patient accounts and their derived graph facts, inferred proposals, revoked access and blocked graph content remain excluded. Existing patient share-confirmation gates remain in force.

**2026-09-20 family graph questions.** The user explicitly authorized family-facing search and Q&A over the knowledge graph, driven by Muse Spark. This supersedes the search/Q&A prohibition in rule 10 and §14 for this new Connections feature. It may retrieve the signed-in household's shared collection, the member's own approved graph contributions, and patient words with a valid share-confirmation and current dashboard access. Answers cite their sources; conversation ideas are suggestions, never stored memories or generated first-person testimony. Unshared call words, other households, revoked access, blocked graph topics, and clinical interpretation remain excluded. Queries and generated answers are not persisted, and the feature cannot write claims or start calls. The legacy redirect tool retains its existing contract.
...
**2026-09-20 shared photo collection integration.** The user explicitly authorized importing the photo collection from `Downloads/hack-mit-26/Relay`, including photo dates/GPS extraction, OpenAI photo organization and transcription, email/password sign-in, family invitation links and opt-in reminders through Linq, and photo storytelling. For this separate household-shared collection, that authorization supersedes the earlier restrictions on those specific features and dependencies. See `docs/PHOTO_FIRST_SCOPE.md`. The existing patient-call graph, evidence and confirmation gates, passive call records, and fixed safety handoffs keep their existing rules. `/caregiver` and `/family` now open the shared collection, `/revisit` opens photo storytelling, and `/conversations` and `/conversations/call` retain call records and scheduled patient calls. Do not infer face identities or turn AI descriptions into anyone's testimony.

**2026-09-20 People and sidebar.** The user explicitly authorized a resizable, collapsible sidebar and facial detection/grouping in a People tab for the shared photo collection. This is a narrow exception to §14's face-clustering restriction: opt-in browser detection suggests unnamed groups within one household, and family members supply any names. Face descriptors, group membership and naming provenance stay in the private collection store; they never create graph identities, relationships, moment participants or reminder recipients. No age, emotion, health or other personal attributes are inferred. Family organizers can clear the face index independently of photos and stories. See `docs/PEOPLE.md` for the pinned open-source models and limitations. All patient-call gates remain in force.

**2026-09-19 pivot.** An earlier version of this file centered Recall on relaying a family member's question to the patient and her answer back to the family. That mechanic is removed. The product now follows the design doc (`SPECS.md`) directly: Recall calls the patient herself, on a schedule, to help her revisit her own memories. If a family member wants to know something, Recall's job is to get them to call her and ask - not to ask on their behalf. See Appendix A for the full reconciliation.

**2026-09-19 revision 2.** The family surface is now specified: family can contribute memories that Recall gently asks her about, receives one passive Weekly Note per week, and can see a per-topic, count-only record of what happened in Recall calls, with a caregiver-initiated export for a doctor. All of it stays inside the non-negotiables below (rules 4, 5, 8, 9, 10, 13, 14). See §6.4 and Appendix A.
...
**2026-09-19 revision 6.** Missed-call tiering and a one-tier caregiver escalation, both extensions of rule 15 / tools 18-19, never of `assess_conversation_state`. After a scheduled attempt that rings and does not connect, a per-patient `missed_call_streak` increments; any call that reaches `connected` resets it to 0. Streak 2 posts a passive, count-only dashboard line ("Susan hasn't answered her last 2 scheduled calls.") - not a Weekly Note, not a push. Streak 3 or more fires `send_safety_alert` with `category: "missed_calls"`. The joint setup may name a `backup_caregiver_id`. Every alert SMS ends with "Reply 1 to let Recall know you've seen this." An inbound `1` (trimmed, case-insensitive) from that caregiver's registered number calls tool 20 `record_alert_ack` and stops the timer. If `ack_timeout_minutes` elapses with no ack, tool 21 `escalate_safety_alert` sends the same fixed text, once, to the backup, and stops. No further tier. Thresholds live in `/fixtures/safety-thresholds.json` and are placeholders, same status as `record-thresholds.json`. The phrase-match handoff itself is unchanged.
...
2. **Retrieve.** Recall periodically calls the person on an ordinary phone. A conversational agent picks a personally meaningful memory from the graph and helps her reach it herself: free recall first, then progressively more context, only as needed. It also learns which cues actually help *her* reach a given memory, and prefers those next time.

Around those two parts, Recall keeps family in the loop without replacing them (§6.4): family contribute memories, see a light weekly note and a per-topic record of what happened in calls, and are pointed back to calling her.
...
2. **No voice cloning.** Her audio is played back, never synthesized. Stored voice is evidence and playback material only, never training or cloning input. When Recall speaks, that voice is clearly labeled as Recall, never as her or a family member.
...
5. **Calls go only to her; family is reached only passively.** Recall places calls only to the patient, only within the call windows, frequency, and topic allow/block list set in the one-time joint setup. Recall never calls, texts, emails, or sends push notifications to a family member, with one exception: the safety alert to a designated caregiver (rule 15). Apart from that, the only channel to family is the passive dashboard: content appears when a member opens it. Recall posts at most one Weekly Note per approved member per 7 days (§6.4.2). It has no mechanism by which a family member can trigger a same-moment call to her.
6. **Evidence-bounded speech.** Recall may verbalize only facts with citations returned by `verify_claim_support`, attributed to their actual speaker. The one exception is the reorientation rung (6.1), which may state a fact outright - still cited, still attributed, never fabricated - and which is never used for an autobiographical or identity memory (`EVIDENCE.md`, section B). Fixed procedural lines carrying no factual claim (6.2) are the only other exception.
...
8. **Data minimization.** Ingest only: the one-time joint setup (approved contributors, call windows, topic allow/block lists, speech pace, max call length, review requirements, designated caregiver(s) and alert channel, confirmation that the number is saved in her phone and that Recall has been introduced to her, dashboard access grants and detail levels), asynchronous family contributions with provenance (a photo, a voice note, a short story, each tagged to its contributor), and from each call the confirmed contribution, literal transcript, accessibility telemetry (response latency, which scaffold rung fired), and the per-topic outcome (topic, highest rung used, whether she reached it unaided, timestamp), plus a safety event if one fires (category and timestamp only, never her words). Delete unconfirmed or declined call audio at call end; only a metadata event (outcome, timestamp) remains. Never ingest continuous recording, health records, location, or unapproved contacts. **The only cross-session view of her is the per-topic record in §6.4. No other analytics, scoring, or comparison surface may exist.**
9. **No medical claims.** Frame Recall as cognitive-accessibility support for memory participation and family connection. No diagnosis, treatment, improvement, retention, or monitoring claims anywhere: code, copy, demo, or write-up. Dashboard copy, the Weekly Note, the change note, and clinician exports describe what happened in Recall calls. They never use the words decline, deterioration, improvement, progress, retention, severity, stage, worse, worsening, or better, and never imply diagnosis or monitoring of a condition. (This is stricter than the design doc's own pitch language - see Appendix B, item 1.)
10. **Her words reach family only through her own share-confirmation.** Recall never answers a family member's question about what she remembers, thinks, or would say, however the question is phrased. Its only response is the redirect line in §1. There is no family-facing search, chat, or Q&A surface; the "Ask about Susan" box is a redirect-only input, not a Q&A surface. Recall never places a call to her on a family member's behalf. Content drawn from her own words may appear on the dashboard only after she hears the exact line played back and confirms she wants it shared (a share-confirmation, distinct from the store-confirmation in rule 3). Observable call facts (a call happened, the topic, which rung was needed) are Recall's statements, not hers, and need no share-confirmation. The fixed-text safety alert (rule 15) never quotes her. Recall never summarizes a call's content to family beyond the observable, non-clinical caregiver receipt (rule 4).
11. **No evaluative or testing language.** Recall never says or displays "wrong," "incorrect," "you forgot," or "try again," and never grades an answer ("right," "good job"). A failed rung is followed by more help, not a judgment. Sessions are supportive, never scored exams.
12. **She can always stop it.** Hang-up, an explicit stop, or a caregiver pause ends the flow immediately from any state: nothing is stored beyond metadata, no follow-up call is placed. Revoking an approved contributor, topic, or call window takes effect before the next call. Revoking dashboard access for any member, or for everyone, takes effect before the next dashboard load, and no further posts are made to a revoked member. A stop never cancels a safety alert that has already been triggered (rule 15). Recall never persuades, retries, or guilt-frames after a stop.
...
15. **Recall is not an emergency service, and it does not sit on a safety concern.** In the joint setup, she and her caregiver name one or more designated caregivers and an alert channel, and may name one backup caregiver. If a final turn of hers contains a phrase on the fixed safety list (`/fixtures/safety-phrases.json`), matched lexically and never inferred, Recall (a) drops the recall flow, (b) says the fixed safety line once, which names the designated caregiver and points her to the local emergency number, and (c) sends one fixed-text alert to the designated caregivers only, stating the category and time and never quoting her. Separately, at the scheduler - never inside the call reducer, and never through `check_safety_phrases` - a scheduled attempt that rings and does not connect increments `missed_call_streak`. Any call that reaches `connected` resets it to 0. At the notice threshold the family dashboard shows a passive count-only line; at the alert threshold the same `send_safety_alert` path fires with `category: "missed_calls"`, stating the streak and the time of the last attempt, never a cause. Every alert SMS includes the reply-1 line. A `1` from the caregiver's registered number acknowledges the most recent pending alert and stops escalation; if the ack window elapses, the same fixed text is sent once to the backup caregiver, and no further tier fires. These are the only exceptions to rule 5's ban on contacting family. A hang-up or stop never cancels an alert already triggered (rule 12), and the alert can be removed only in the joint setup, never by anything said during a call. Recall never claims, in code, copy, demo, or write-up, to monitor her between calls, detect emergencies, or replace emergency services.
16. **Recall is honest about what it is, and looks trustworthy to her.** The first line of every call states, in plain words, that Recall is an AI assistant set up by a named family member (fixed script line, name filled from setup). Any time she asks who or what Recall is, or whether it is a person, it gives the fixed identity line ("I'm Recall, a computer assistant, not a person. Maya set me up to keep you company."). Recall never claims to be a person or a family member. Before the first call, the number must be saved in her phone under a family-chosen name and photo, and a family member must have introduced Recall to her (setup attestations, enforced by `place_recall_call`). Recall never asks for or discusses money, account numbers, passwords, addresses, or other identifiers, and never asks her to do anything except talk.
...
- **8-20s**: Her phone rings. The screen shows a saved contact, "Recall (from Maya)," with a family-chosen photo. She answers. Recall: "Hi Susan, I'm Recall, an AI assistant Maya set up to keep you company. I'd love to hear about the summers at Cape May. What comes to mind?" Recall waits through a held pause of about six seconds and does not fill it. She says, "Cape May...?" No red error, no diagnosis label, no transcript wall, no visible trace to her.
...
| Second `lost` signal on a topic | -> `no_answer_today`; graceful wrap-up | unconfirmed audio deleted at call end |
...
| Identity, evidence, or policy missing or mismatched | flow stops; Recall falls back to the narrowing line (rule 7) | nothing further ingested |
...
| She asks who or what Recall is, or whether it is a person | fixed identity line, then the flow continues or closes | none |
...
16. `export_record_for_clinician(requester_id)` -> approved-member-initiated export of the same per-topic content plus the fixed non-clinical note. Logged. Recall never sends it to anyone.
...
18. `check_safety_phrases(final_turn)` -> deterministic lexical match of her final turn against `/fixtures/safety-phrases.json`; returns a category or none. Runs first on every final turn of hers, before any other processing. Never a model judgment, never applied to Recall's own speech. Expect false positives; a neutral alert is the cost of avoiding false negatives.
19. `send_safety_alert(category, caregiver_ids)` -> sends the fixed alert text for the category, through the channel chosen in setup, to the designated caregivers only. At most once per category per call. Contains the category and time, never her words or audio. Appends the reply-1 line. Logs a pending `SafetyEvent` and starts the ack timer when a backup caregiver is named. Not a Weekly Note and not subject to its cap. Used for a phrase match and for `missed_calls`.
20. `record_alert_ack(alert_id, caregiver_id)` -> called when an inbound SMS of `1` (trimmed, case-insensitive) arrives from a caregiver's registered number while they have a pending, un-acknowledged alert. Marks the `SafetyEvent` acknowledged and stops the escalation timer. Idempotent: a second `1`, or a reply after ack or escalation, does nothing and sends no response text.
21. `escalate_safety_alert(alert_id)` -> fires only if `ack_timeout_minutes` elapses with no ack. Sends the same fixed alert text, unchanged category, to `backup_caregiver_id` by SMS. One backup tier only. No-op when no backup is set, or the alert is already acknowledged or escalated.
...
| 5 | **Reorientation**: offers a fact directly - for **procedural or functional information only**, framed as offering and never as correcting. **Never used for an autobiographical or identity memory**: for those the ladder ends at rung 4 and the call closes kindly | "This button plays your message." (Recall has no procedural topics today, so its script has no rung-5 line at all.) |

Rules:

- Climb one rung at a time; never jump straight to reorientation. Log every rung tried, in order, with its outcome.
- **Recall never states an autobiographical or identity memory outright** (`EVIDENCE.md`, section B). Errorless learning, the evidence usually cited for a "state the fact" rung, covers new procedural learning, not a fact she already knows and cannot reach; and repeated correction is the documented harm in reality orientation. A topic category in `call-script.json` declares `memory_kind`; only a `procedural` one may carry a reorientation line, and the script schema, `select_scaffold`, and the reducer each refuse it otherwise. If rung 4 does not reach it, Recall closes kindly and the memory comes round again on a later call.
- **Invitations, not identification questions.** Rung-1 prompts are open invitations ("I'd love to hear about...", "What comes to mind when you think of..."). Recall never opens with "Who is...?", "Do you remember...?", or any identification or yes/no test. Place, event, and story nodes are the default topics. A person node is a topic only if the caregiver enables that in setup, and is then opened as an invitation ("I'd love to hear about Maya").
...
### 6.2 Call conduct (Recall's voice)

- **Identity.** Caller ID and the saved contact name Recall and show a family-chosen photo. The first line of every call says in plain words that Recall is an AI assistant set up by a named family member, then invites her to talk about the topic warmly, never diagnostically (see the golden-path opener). If she ever asks who or what Recall is, or whether it is a person, it gives the fixed identity line (rule 16) and then continues or closes, as she wishes.
- **What Recall never asks.** Money, account numbers, passwords, addresses, or any other identifier, and it never asks her to do anything except talk.
- **Pace and shape.** One idea and one question at a time, plain words. No jargon, no lists read aloud. Simplify by shortening sentences and cutting subordinate clauses, not by slowing down: it is syntactic complexity, not speech rate, that costs comprehension (`EVIDENCE.md`, section C). The lint enforces at most one question per line and at most 16 words per sentence, from `call-script.json`.
- **When, and for how long.** The joint setup defaults to morning call windows, and a call is 8 minutes by default and never more than 10 (`EVIDENCE.md`, section E). A family may widen the windows; the setup schema will not accept a longer call.
- **What a yes is worth.** Recall logs the kind of prompt beside every reply (`open`, `forced_choice`, `yes_no`), because a yes/no answer is the format most open to a yes that means no. A memory is never stored on a bare yes: she has just heard her own words played back.
- **Fixed procedural lines.** Every line that carries no factual claim (greeting, topic-intro template, each ladder-rung template, store-confirmation request, share-confirmation request, thanks, kind close, narrowing line, restate-once fallback, family-redirect line, identity line, safety line) lives in `/fixtures/call-script.json` with a stable script ID. Template slots are filled only from graph node values. Every other Recall line must carry citations (rule 6).
...
Alongside the memory graph, Recall keeps a second, thinner layer: which rung and which specific cue actually got her to a memory, per topic node - the design doc's "Maya: useful cue Cape May; ineffective cue wedding year."

- **What it stores:** per topic node, per cue candidate - a rung id, an effectiveness count, and a last-used timestamp. Nothing else.
- **What it's for:** when rung 3 or 4 needs to pick which candidate cue to offer, it prefers one the retrieval layer marks effective for her. It never decides *whether* to climb the ladder - only *which* cue to reach for once a rung is already warranted by 6.1.
...
- **Controls:** editable and deletable by her caregiver at any time from setup; deleting it resets Recall to plain rung-by-rung climbing with no cue preference. Deleting it does not delete the per-topic record, and vice versa; they are independent stores.
...
**6.4.1 Family contributions ("Tell Recall about a memory").**

- A separate, clearly labelled, one-way form: "Tell Recall about a memory you share with Susan." It cannot return anything from the graph. Fields: who, what happened (a few sentences), roughly when/where (optional), optional photo. Stored via `receive_family_contribution` with contributor, medium, timestamp, and `patient_confirmed: false`.
- If text entered here begins as a question, the form rejects it with a short hint: "This box is for telling Recall something you remember. To find out what Susan remembers, give her a call." This keeps the redirect-only "Ask about Susan" box (rule 10) and the contribution form from being confused.
...
- If she elaborates, the elaboration goes through capture and store-confirmation like any other contribution. Her account and the family member's stay separate, linked claims. If they differ, they are linked by `CONTRADICTS`, neither is spoken, and the difference may appear as a difference prompt in the Weekly Note (6.4.2). Recall never says which account is right.

**6.4.2 The Weekly Note (the only thing Recall posts).**
...
1. **A warm line, topic-only and observable.** Example: "Recall talked with Susan about the summers at Cape May this week. Want to give her a call?" Not "She was thinking of you" - that claims her inner state (rule 4).
...
3. **One gap or difference prompt** (at most one): a gap ("Cape May has no year yet. Do you remember one?") or a difference ("Susan and Maya remember this differently. Want to call her about it?"). Recall never says which account is right.
...
- Fixed header, always visible: **"This is a record of what happened in Recall calls, not a measure of Susan's memory overall. Practice on a topic, call quality, and time of day all affect it."**
...
- **Summary line.** If 3 or more topics show a topic line in the direction of less unaided recall, show this fixed line (`FAM-CHG-01`): **"In recent Recall calls, more prompting was needed across several topics than in earlier calls. This is a record of Recall calls, not a clinical assessment. If you have concerns, you can export the record to share with a doctor."**
...
- Comparison is only against her own earlier Recall calls, never against a population norm or a clinical cutoff (rule 4).

**6.4.5 Clinician export.**

`export_record_for_clinician` is initiated by an approved member. It produces a file of the per-topic counts, dates, any change lines, and the fixed header, plus: "This record is not a clinical assessment or diagnosis." Recall never sends the file to anyone. The export is logged (who, when) and visible to all approved members.

**6.4.6 Missed-call notice (a view, not a post).**

The streak is the count of trailing `not_answered` Session records. When it is at or above the notice threshold, the dashboard shows one count-only line (`FAM-MISS-01`): "Susan hasn't answered her last 2 scheduled calls." Same tone and placement as a Weekly Note gap/difference prompt - plain text, no push, no urgency styling. It counts toward nothing: not the Weekly Note cap, not a SafetyEvent. A later connected call clears the streak and the line. The SMS alert at the higher threshold is the rule-15 path, not this line.
...
/lib/tools           the 21 tool implementations + JSON schemas + gates
...
/fixtures            deterministic tool outputs, graph seed, policy seed,
                     ...
                     safety-thresholds.json (missed-call and ack numbers),
...
A live model + live Deepgram/Muse path may exist as an optional side demo, behind a flag, never in the judged path. The counterfactual (a version without the ladder, where Recall repeats "Who is Maya?" once and she says "I don't know") is prerecorded and clearly labeled as an alternate path, never as a claim about the person.
...
- **Right rail (24%):** "How Recall helped." Human-readable event cards (Topic selected; Cue given; Recalled; Remembered - added to graph). Tool names as tiny monospace labels for judges.
- **Bottom:** one scrubber with the chapters from §4, including the family-redirect beat and the family dashboard.
- **Authorship ribbon:** a thin coral line that starts at whichever graph node the call is about, crosses center only when she speaks, and ends wrapped around the new contribution card. It never touches Recall's scaffold words.
...
- **Family app views (never shown to her):** (1) "Tell Recall about a memory," a one-way form; (2) the Weekly Note at the top of the dashboard, empty state "Nothing new this week"; (3) the per-topic record, plain counts, fixed header, change lines when active, and a secondary "Export for a doctor" button; (4) the small "Ask about Susan" redirect-only box, visually distinct from the contribution form; (5) the safety alert card, shown only to a designated caregiver: fixed text, category and time, and a "Call Susan" button. No push indicators, unread badges, streaks, or urgency styling on any family view, except the safety alert card (rule 15).
...
**Information hierarchy:** person/topic -> current cue or answer -> whether Recall is listening, helping, or waiting for confirmation -> provenance/trace. Never surface model confidence, clinical labels, or caregiver analytics in the primary view.
...
8. **Language and conduct**: the lint over `call-script.json`, `family-copy.json`, every rendered Recall line, all UI copy, the Weekly Note, the record header, the change lines, the export note, and the receipt finds no banned evaluative phrase (rule 11), no diagnostic or emotional-state word (rule 4), and none of the rule 9 words. Family-sourced rung-3 prompts must end in an open question (no yes/no phrasing). Rung-1 prompts must be invitations and never begin with "Who is" or "Do you remember"; no Recall line may ask for money, account numbers, passwords, addresses, or identifiers. Each rung fires at most once per topic; every rejected lower rung is logged with a reason.
...
- Changes to `/lib/state`, the tool JSON schemas, policy fixtures, `call-script.json`, `family-copy.json`, `record-thresholds.json`, `safety-thresholds.json`, or `safety-phrases.json` require a second person's review: they define the safety gates, what Recall may say, and what the family may be shown, including the family-redirect guarantee.
...
- **Relaying a family member's question to her, or her answer back to family, in any form.** This was the product's earlier direction and is now explicitly out of scope (Appendix A). Recall's only response to a family question is the redirect line.
...
- Diagnosis, staging, mood detection, cognition scoring, aggregate or severity figures, "decline," "improvement," or "progress" language, and any alert that compares her to a population norm or a clinical threshold. The per-topic record (§6.4) is counts-only; change lines compare her only to her own earlier Recall calls by the fixed rule in §6.4.4; the clinician export is member-initiated and labelled non-clinical.
...
- Emergency dispatch, fall detection, location, wearables, between-call monitoring, or any claim that Recall is an emergency service. The safety handoff is a fixed-phrase alert to a designated caregiver, nothing more.
...
- The first line of every call discloses that Recall is an AI assistant; Recall never asks for money or identifiers.
...
| Family memory contributions prompting Recall to ask her | **Adapted:** stored as the contributor's unconfirmed claim, rungs 1-3 only, attributed, open questions. | rule 13; §6.1, §6.4.1 |
...
| Family fact-checking of memories | **Adapted:** differences surface as "remember this differently - want to call her?" Recall never resolves them. | §6.4.1, §6.4.2 |
...
| No contact with family by Recall at all (rule 5 as written in revision 2) | **Narrowed:** one exception, a fixed-text safety alert to the designated caregivers only, agreed in setup. | rules 5, 12, 15 |
| Recall introduces itself only as "Recall" | **Extended:** AI-assistant disclosure on every call, saved-contact and introduction attestations, and never asking for money or identifiers. | rule 16; §6.2 |
...
4. **Thresholds are not clinically validated.** The record windows (last 8 calls), minimums (3 calls to show, 8 to compare), the difference of 3, and the 3-topic summary minimum live in `/fixtures/record-thresholds.json`. The missed-call notice/alert counts (2 and 3) and the ack timeout (10 minutes) live in `/fixtures/safety-thresholds.json`. All of these are placeholders, and may be changed later with a second reviewer.
...
9. **Backup does not ack.** If the backup caregiver also does not reply `1`, nothing further happens automatically. Do not build a third tier, a retry, or an operator page around that silence until the team decides.
10. **More than one pending alert for the same caregiver.** A reply of `1` acknowledges their single most recent pending alert. Do not build multi-alert disambiguation until it comes up.
