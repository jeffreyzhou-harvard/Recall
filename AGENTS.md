...
- **Primary sponsor: Meta, "Bringing People Closer Together with AI."** Meta integration must be load-bearing, and it is Muse Spark (`muse-spark-1.3`, section 20): it makes the scaffold decision from typed tool outputs and must cite graph node IDs in its response, and it reads facts out of answers for graph ingestion (section 18), where `applyAnswer` decides what is kept. **Muse Voice Transcribe is not used**: it returns turn-level timestamps only, and the provenance receipt needs word timings, so speech-to-text is Deepgram; Relay's policy check, graph query, assent, and publish actions are exposed as tools/connectors; publish hard-fails on policy mismatch or missing assent. Do NOT integrate the Facebook Graph API unless a real, user-consented path works; a mocked Facebook surface hurts credibility. Meta submission needs: working prototype, 2-3 minute demo video, public repo, short write-up naming the user, the connection benefit, and why AI is essential.
...
- **8-20s**: Video call, full-screen and warm. Relay: "Anika wants your help with Diwali dessert." Mom smiles, then asks, "Which thing again?" No red error, no diagnosis label, no transcript wall, no visible trace.
...
**Stack:** Next.js (App Router) + TypeScript + Tailwind + Framer Motion + Zustand (or a tiny reducer) + Lucide icons. LadybugDB for the graph. Deepgram streaming STT (her words and their timings) and Meta Muse Spark (scaffold choice, graph ingestion) behind the tool interfaces above; see section 20. All media is local. No backend is required for the judged path. The family's thread is Telegram, through the Bot API over plain `fetch` (no SDK) - live only, never on the judged path; see section 17.
...
/lib/providers       transcription boundary (fixture on the judged path); deepgram.ts and muse/ are live only
...
- **Center stage (48%):** "With Mom now." Large video/portrait loop, her name, and "Anika is asking about Diwali." One cue card at a time: the photo, asker portrait, then two large labeled choices. No transcript crawl while she speaks. A small "Video call" badge so judges know the only thing she ever does is answer it: one tap, then she talks.
...
**Decided: the call is a video call, not a phone call.** Earlier drafts of this file said "an ordinary phone"; sections 1, 4, and 10 now say video. She is on a browser device, so what "she never operates this UI" has to mean is this: her page (`/call/[room]`) is one task with one large control, someone may open the link for her, and after that there is exactly one thing to tap. It already keeps section 10's hard rules (24px+ text, 44px+ control, plain words, no technical state, no countdowns, no red errors) and must keep them when it is designed properly.
...
- **What drives a session over this call** is section 20: WebRTC is only the transport and the capture.
...
## 20. The live session: video call + Deepgram + Muse Spark (live only)

A granted ask places a video call and runs the same orchestrator, tools, gates, and state machine as the judged path. Only the `CallDriver` and the providers differ. `RELAY_CALL=video` turns it on; it needs `DEEPGRAM_API_KEY`, and uses Muse Spark when `MUSE_API_KEY` is set.

- **The session runs on the server; Relay's call page is its mouth and ears.** `server/live-calls.ts` places the call and announces it; the operator's open `/call/host` page joins as Relay's media endpoint and shows her one-time link. Her audio goes browser -> `POST /api/call/[room]/audio` -> `LiveCall` (`lib/call/live-call.ts`) -> Deepgram. Commands come back over SSE (`/control`): say this line, play back these spans, hang up. The web app asks through `POST /api/live/asks`; Telegram asks through the webhook. Both reach the same `getLiveRelay()` singleton, because the call must be placed by the process that serves the call pages - locally that means `npm run telegram -- poll --forward http://localhost:3000`, not a bare `poll`.
- **The clock is her audio.** Call time is counted in samples received, so turn timings, Relay's speaking marks, the kept spans, and the WAV cuts all share one clock, and tests need no timers.
- **Relay's voice** is the device's own synthetic voice (`speechSynthesis`) on HER page, sent as text over an `RTCDataChannel` and shown labeled "Relay". It is never a cloned voice and never audio presented as hers (rules 1-2). **Playback is her own PCM**, played into the call by Relay's page: exactly the kept spans of the pending artifact, so what she approves is what is delivered.
- **Relay never takes its own voice for hers.** Each line marks the stretch of call time it occupied (ending when her page says it finished, plus a short tail); anything heard inside a mark is discarded as echo. The mark's end is read when the acknowledgement ARRIVES - reading it later swallows the start of her reply.
- **Deepgram** (`lib/providers/deepgram.ts`, nova-3, streaming): the key travels as a WebSocket subprotocol, never in a URL; only `is_final` words are used; a turn closes on `speech_final` or `UtteranceEnd`, so the state machine always classifies a finished turn. Graph labels are sent as keyterms so family names and dishes are heard correctly. Word timings become the transcript spans in the receipt.
- **She is never hurried.** Relay waits 12 s of quiet after it stops talking, never counts quiet while she is mid-sentence, and shows no countdown anywhere. Silence is a lost-thread signal, not a failure: it leads to one re-anchor and then a kind wrap-up, never to sending.
- **A dropped call claims nothing.** `CALL_DROPPED` ends in `blocked` (nobody joined), `wrapped_up` (dropped mid-call), or `not_sent` (dropped after capture, before her yes). Nothing is delivered without her assent, and the family gets the fixed "not this time" notice.
- **Rule 8 holds on the server too.** Her audio is held in memory for the length of the call and zeroed at hang-up, delivered or not; only the approved cut survives, as the delivered artifact. Video never leaves the peer connection. `RELAY_CALL_DEBUG=1` logs levels and turn timings - never her words.
- **Muse Spark only proposes** (`lib/providers/muse/`). Scaffold choice: Relay's ladder first works out which rungs are ELIGIBLE; Spark picks among those and must cite that rung's verified node ids; a pick outside them, a failure, or no reply inside the budget (6 s - she is waiting on the line) means the ladder's own choice is spoken, and the tool output records `decided_by`. Graph ingestion: Spark's proposed facts go through `applyAnswer` like anyone else's, so a name she never said is refused. Spark is only ever shown confirmed people, so it cannot launder an inference into a fact (rule 10).
- **Two things the Meta docs get wrong or leave out, verified against the live API:** `reasoning_effort: "none"` is rejected (use `"minimal"`), and Spark's hidden reasoning is billed against `max_completion_tokens` - set it too low and the reply comes back EMPTY with `finish_reason: "length"`. Leave thousands, not hundreds.
- `npm run build && npm run e2e:live` runs the whole thing for real: two headless Chromes, a synthetic voice on her microphone, real Deepgram, real Muse Spark, ending in a delivered verbatim contribution. It costs a few cents a run, needs macOS `say` and Chrome, and is not part of `npm run check`. Chrome's sandboxed audio service cannot read the microphone file on macOS and plays silence without saying so; the script turns that sandbox off for the test browser only.
