...
- **Capture.** The person and the people who know her contribute memories - photos, voice, simple questions, family stories - while those memories are still accessible. The graph organizes this into people, relationships, places, events, stories, and preferences, each with visible provenance: who said it, and when.
- **Retrieve.** Relay periodically calls the person on an ordinary phone. A conversational agent picks a personally meaningful memory from the graph and helps her reach it herself: free recall first, then progressively more context, only as needed. It also learns which cues actually help her reach a given memory, and prefers those next time.
...
```
idle -> scheduled -> policy_passed -> connected -> topic_selected -> asking -> lost -> reanchored -> recalled -> confirming -> confirmed -> stored
```
...
1. `get_next_recall_topic(person_id, schedule_context)` -> the person/place/event/preference node due for revisit, ranked by freshness and the retrieval layer (6.3), constrained by the topic allow/block list. Deterministic ranking; the model does not free-pick a topic. Family-sourced, unconfirmed nodes are eligible (6.4.1). Default topics are Place, Event, and story nodes; a Person node is a topic only if the caregiver enables that in setup (6.1).
...
| 5 | Reorientation: states the fact directly, warmly, attributed to her own prior words where possible, followed by an open question to re-engage rather than ending on the assertion | "You and your daughter Maya spent summers together at Cape May. You told me about them. What do you remember about those?" |
...
- **Pace and shape.** One idea and one question at a time, plain words, speech pace from setup. No jargon, no lists read aloud.
...
- **Controls:** editable and deletable by her caregiver at any time from setup; deleting it resets Relay to plain rung-by-rung climbing with no cue preference. Deleting it does not delete the per-topic record, and vice versa; they are independent stores.
...
**Open-source basis:** Kuzu (embedded property graph, Cypher, MIT) for persistence. Graphology (MIT) for the animated judge view, not persistence. W3C PROV-O vocabulary for provenance concepts (attribute the spec). Schema.org vocabulary where useful (CC BY-SA 3.0; keep extensions in a separate Relay namespace with attribution). Wikidata (CC0) only for public place/entity IDs. Never import personal or health assertions.
...
**Stack:** Next.js (App Router) + TypeScript + Tailwind + Framer Motion + Zustand (or a tiny reducer) + Lucide icons. Kuzu for the graph. Deepgram streaming STT and Meta Muse calls behind the tool interfaces above. All media is local. No backend is required for the judged path.
...
/lib/graph           Kuzu schema, seed loader, citation queries, retrieval layer,
...
- **5-8:** Kuzu schema + 20-30 seeded facts, artifact hashes and citations, policy query, retrieval-layer table, TopicOutcome seed and `record-thresholds.json`.
...
## Appendix C. Implementation notes (added by the implementing agent, 2026-09-19)

The sections above are the brief as the team wrote it. This appendix records where the code differs from their wording, and every place the brief was silent or disagreed with itself and a reading had to be chosen. Each is a decision the team can overturn; none was made silently.

**Standing decisions the user made on 2026-09-19 that the text above does not reflect:**

1. **The graph database is LadybugDB** (`@ladybugdb/core`, Kuzu's successor), not Kuzu, which was removed entirely at the user's direction. Read "Kuzu" in §7, §8, and §11 as LadybugDB.
2. **Speech-to-text is Deepgram**, not Muse Voice Transcribe (§3): Voice Transcribe returns turn-level timestamps only, and the receipt needs word timings. Muse Spark is the Meta integration.
3. **The live side demo's call is a WebRTC video call** (`lib/call`, `/call/host`, `/call/[room]`), because no telephony exists. The judged path is still a prerecorded call and needs neither. §1, §4, and §10 say "ordinary phone"; rule 16's saved-contact attestations are enforced either way.

**Readings chosen where the brief is silent or inconsistent:**

4. **"Second lost signal" (§5) means two windows in which she said nothing at all.** §4's golden path has two spoken misses and then a hit, so a spoken miss cannot be a "lost signal": it climbs the ladder. `no_answer_today` is also where an exhausted ladder ends.
5. **`confirm_and_store` has two steps, `confirm` and `commit`,** so that commit can come last (§5) without a twentieth tool. `commit` is refused until the share question has resolved.
6. **Topic selection is deterministic (tool 1); Muse Spark never picks a topic or a rung** (§3 says it does both). It may pick WHICH cue, and only where the retrieval layer has no preference between the front-runners; its pick is re-checked and falls back to the deterministic one.
7. **She reaches it and offers nothing more:** ends in `not_stored` with a kind close. The TopicOutcome still records the rung she reached it at.
8. **A call that ends `stopped` or `safety_handoff` writes a Session and nothing else:** no TopicOutcome and no cue record, so an interrupted call is never counted as a miss.
9. **`first_rung_reached_unaided`** holds the rung at which she reached the memory (1 = free recall, unaided), or null.
10. **Redirect line or "nothing yet" (tool 12)** is chosen by one boolean about the whole graph, never by the question, so the box cannot be used to probe whether she has said anything about a particular thing.
11. **Rung 4** orders its two options by the other person's id (a rule that knows nothing about which is in the memory), and logs a cue record like rung 3.
12. **`WORKED_AT` and `LOCATED_AT`** are `RELATED_TO` edges with `relation: worked_at | took_place_at` from the closed vocabulary in `lib/graph/relations.ts`, which also keeps the word she actually used (`said_as: "daughter"`).
13. **The agreed call length** turns the next conversational step into the kind close, but never cuts off a confirmation already under way.
14. **The family-redirect lines live in `call-script.json`** (as §6.2 lists them); the receipt's fixed lines live in `family-copy.json`. One lint covers both.
15. **Appendix B item 8 is not implemented:** nothing stops a reply that echoes Relay's cue from being stored. (An echo of Relay's own words IS read as a rung miss, which §4 requires for "Cape May...?".)

**Not built, and why:**

16. **Nothing creates a `CONTRADICTS` edge.** The graph, `verify_claim_support`, and the Weekly Note's difference prompt all honour one, but deciding that two accounts differ is not something a lexical rule can do, and family cannot see her claims to flag one. The team needs to say who decides.
17. **The seed's "ineffective cue: the year"** is not represented: cues are people and photographs today.
18. **There is no sign-in.** The family routes (`/api/family/*`) take `member` from the caller and need the operator secret in production. What a member may see is still decided by the joint setup on every load.
19. **`/components` and the `/family` app are not built** (frontend was deferred). `/present` is a plain engine check of the whole golden path, family beats included.

**Still in the repo, out of scope under §14:**

20. **`legacy/`** holds the family-relay mechanic (intake, thread bridge, Telegram bot, their tests and fixtures), moved there unchanged and excluded from the build. Delete it once the team agrees: `git rm -r legacy`.
21. **`lib/discovery`** still contains photo-library ingestion with face clusters, which §14 now rules out. It is off unless the joint setup turns it on (`discovery.enabled`, false in the judged setup). Its question-and-answer half is the "ask, don't assert" path of §7 and should be kept if the ingestion half is removed.
