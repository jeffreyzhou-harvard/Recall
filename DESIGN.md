...
One book represents one persisted dated topic call. Session metadata comes through `lib/family/session-summary`; no graph claims or personal words enter this projection. The server supplies the record window and minimum-history count. Resting scale is `MEASURED_FLOOR + (1 − MEASURED_FLOOR) × clamp(unaided / recordWindow, 0, 1)` with a floor of .6; unmeasured scale is .55, just below the shortest measured book. With the default eight-call window this is 221px plus about 18px per unaided call at desktop size, so the shortest measured spine keeps three fifths of the tallest one's height. Every measured spine is ink; insufficient history is dashed. Always retain the count denominator. The cover expands for reading, not extra quantitative meaning.
