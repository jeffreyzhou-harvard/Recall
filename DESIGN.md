...
One book represents one persisted dated topic call. Session metadata comes through `lib/family/session-summary`; no graph claims or personal words enter this projection. The server supplies the record window and minimum-history count. Resting scale is `MEASURED_FLOOR + (MEASURED_CEILING − MEASURED_FLOOR) × clamp(unaided / recordWindow, 0, 1)`, a .5 floor to a .82 ceiling; unmeasured scale is .45, just below the shortest measured book. With the default eight-call window this is 184px plus about 15px per unaided call at desktop size, so the shortest measured spine keeps three fifths of the tallest one's height and every closed spine stays well under the opened cover. Every measured spine is ink; insufficient history is dashed. Always retain the count denominator. The cover expands for reading, not extra quantitative meaning.

Native scrolling rotates the nearest cover from −90° toward −8°. Bottom-centered transforms preserve the baseline. Cached geometry and frame updates touch the visible band only. Touch, wheel, clicking, previous/next and Arrow/Home/End select the same record; wheel returns to page scrolling at boundaries. Topic filters are separate underlined text controls. A noninteractive date timeline places its Clay marker by elapsed calendar time. Two legend symbols name what each encoding means: taller bars for calls that needed no cue, and a dashed spine for a topic under the server-supplied minimum.
