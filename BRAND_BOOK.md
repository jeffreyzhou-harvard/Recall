...
One book opens at a time. Binding rules and a fine page edge are enough to suggest a book; omit leather textures and ornaments. At desktop size, resting heights range from 184px for zero unaided calls to 302px for eight, with about 15px per additional call. The shortest measured spine keeps three fifths of the tallest one's height, so a difference of one or two calls never reads as a dramatic one, and even the tallest closed spine stays well under the opened cover. Every measured spine is ink: height and the exact printed count carry the meaning, and no shade divides the shelf into better and worse books. The open cover grows for readability, and its displayed count remains the quantitative explanation. Support descriptions and dates are sufficient for the summary. Do not expose raw transcripts, private quotes, or a queryable memory graph in this record.

Use two small visual legend keys, each naming what its encoding means in full: **taller bars + “Taller book: more of these calls needed no cue”**, and **an outlined dashed spine + “Dashed spine: fewer than 3 calls, not counted yet.”** Every visual difference on the shelf must be named by a key; do not add an encoding the legend does not explain, and do not append a supporting line the keys already cover. Avoid an explanatory dropdown. Keep topic filters separate from the legend and avoid giving each category its own colored panel.
