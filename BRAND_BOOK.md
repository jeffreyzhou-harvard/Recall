...
One book opens at a time. Binding rules and a fine page edge are enough to suggest a book; omit leather textures and ornaments. At desktop size, resting heights range from 221px for zero unaided calls to 368px for eight, with about 18px per additional call. The shortest measured spine keeps three fifths of the tallest one's height, so a difference of one or two calls never reads as a dramatic one. Every measured spine is ink: height and the exact printed count carry the meaning, and no shade divides the shelf into better and worse books. The open cover grows for readability, and its displayed count remains the quantitative explanation. Support descriptions and dates are sufficient for the summary. Do not expose raw transcripts, private quotes, or a queryable memory graph in this record.
